...
use NetlifyConnect\Utils\Utils;
...
class AcfMonitor extends Monitor {
	...
	public function init() {
		...
		add_action(
			...
			function () {
				$this->trigger_schema_diff(
					[
						'title' => __('Update ACF Field Group', 'NetlifyConnect'),
					]
				);
			}
		);
		...
		add_action(
			...
			function () {
				$this->trigger_schema_diff(
					[
						'title' => __('Delete ACF Field Group', 'NetlifyConnect'),
					]
				);
			}
		);
	...
	public function after_acf_save_post() {
		...
		$option_pages_slugs = apply_filters(
			'netlify_action_monitor_tracked_acf_options_pages',
			$option_pages_slugs
		);
