...
class PostTypeMonitor extends Monitor {
	...
	/**
	 * @var array List of registered post types that Netlify Connect tracks
	 */
	...
	public function init() {
		...
		$this->option_name = '_netlify_tracked_post_types';

		// Check to see if the post types are different
		add_action('netlify_init_action_monitors', [$this, 'check_post_types'], 999);
	}
	...
	public function check_post_types() {
		...
		if (!empty($added)) {
			$this->trigger_schema_diff(
				[
					'title' => __('Post Type added', 'NetlifyConnect'),
				]
			);
		}
		...
		if (!empty($removed)) {
			$this->trigger_schema_diff(
				[
					'title' => __('Post type removed', 'NetlifyConnect'),
				]
			);
		}
	}
