...
class TaxonomyMonitor extends Monitor {
	...
	/**
	 * @var array List of registered taxonomies that Netlify Connect tracks
	 */
	...
	public function init() {
		...
		$this->option_name = '_netlify_tracked_taxonomies';

		// Check to see if the taxonomies are different
		add_action('netlify_init_action_monitors', [$this, 'check_taxonomies'], 999);
	}
	...
	public function check_taxonomies() {
		...
		if (!empty($added)) {
			$this->trigger_schema_diff(
				[
					'title' => __('Taxonomy added', 'NetlifyConnect'),
				]
			);
		}
		...
		if (!empty($removed)) {
			$this->trigger_schema_diff(
				[
					'title' => __('Taxonomy removed', 'NetlifyConnect'),
				]
			);
		}
	}
