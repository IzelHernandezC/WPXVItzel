...
class PostMonitor extends Monitor {
	...
	public function callback_transition_post_status($new_status, $old_status, WP_Post $post) {
		...
		// to let Netlify Connect know it should no longer cache the post
		if ('publish' !== $new_status && 'publish' === $old_status) {

			$action_type = 'DELETE';

			// If a post that was not published becomes published, set the action_type to create
			// to let Netlify Connect know it should fetch and cache the post
	...
	/**
	 * Log a user update when a post is created or deleted, telling Netlify Connect to
	 * invalidate user caches
	 *
	 * @param WP_Post $post The post data of the Post being updated
	 *
	 * @todo:
	 *      This should be able to be removed at some point as Netlify Connect
	 *      _should_ be able to handle bi-directional relationships implicitly. When a Post is
	 *      created, Netlify Connect queries the full post fields, including the Author.node.id, and should
	 *      be able to handle the relationship between the new post and the author. When a post is
	 *      deleted, Netlify Connect should remove the post node and any queries (such as author archive pages)
	 *      that include references to the deleted post node should automatically be updated by Netlify Connect.
	 */
